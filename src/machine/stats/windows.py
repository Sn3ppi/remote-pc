...
from app.config.const import OHW_DLL

clr.AddReference(OHW_DLL)
...
def get_temperatures() -> list:
    '''Получает температуру процессора и видеокарты.'''
    c = Computer()
    c.CPUEnabled = True
    c.GPUEnabled = True
    c.Open()
    values = [None, None]
    sensors = [sensor for device in c.Hardware for sensor in device.Sensors if sensor.SensorType == sensor.SensorType.Temperature]
