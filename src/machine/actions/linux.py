...
def name_validation(path: str) -> str:
    ...
    default_file_name = f'file_{datetime.datetime.now().strftime("%d_%m_%Y_%H_%M_%S_%f")[:-3]}'
...
def start_app(idx: int) -> None:
    ...
    try:
        os.system(f"xdg-open {os.path.abspath(os.path.join(favorites_dir(), data[idx]))}")
